...
export const add = callable(class extends BinaryFunc {
    ...
    derivative(sym) { return add(this.args[0].diff(sym), this.args[1].diff(sym)); }
})

export const sub = callable(class extends BinaryFunc {
    constructor() { super(...arguments); }
    repr() { return `\\left(${arguments[0]}\\right)-\\left(${arguments[1]}\\right)`; }
    derivative(sym) { return sub(this.args[0].diff(sym), this.args[1].diff(sym)); }
})

export const mul = callable(class extends BinaryFunc {
    constructor() { super(...arguments); }
    repr() { return `\\left(${arguments[0]}\\right)\\cdot\\left(${arguments[1]}\\right)`; }
    derivative(sym) { return add(mul(this.args[0].diff(sym), this.args[1]), mul(this.args[0], this.args[1].diff(sym))); }
})

export const div = callable(class extends BinaryFunc {
    constructor() { super(...arguments); }
    repr() { return `\\frac{${arguments[0]}}{${arguments[1]}}`; }
    derivative(sym) { return div(sub(mul(this.args[0].diff(sym), this.args[1]), mul(this.args[0], this.args[1].diff(sym))), pow(this.args[1], integer(2))); }
})

export const pow = callable(class extends BinaryFunc {
    constructor() { super(...arguments); }
    repr() { return `${arguments[0]}^{${arguments[1]}}`; }
    derivative(sym) { return add(mul(this.args[0].diff(sym), mul(this.args[1], pow(this.args[0], sub(this.args[1], integer(1))))), mul(this.args[1].diff(sym), mul(pow(this.args[0], this.args[1]), ln(this.args[0])))); }
})

export const log = callable(class extends BinaryFunc {
    constructor() { super(...arguments); }
    repr() { return `\\log_{${arguments[0]}}\\left(${arguments[1]}\\right)`; }
    derivative(sym) { return div(sub(mul(div(this.args[1].diff(sym), this.args[1]), ln(this.args[0])), mul(div(this.args[0].diff(sym), this.args[0]), ln(this.args[1]))), pow(ln(this.args[0]), integer(2))); }
})