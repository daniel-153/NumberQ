...
const REH = { // genRatEx helpers
    add_sub: { // addition and subtraction templates
        as_1: {
            global_reqs(a,b) {
                return (
                    a > 0 &&
    ...
    mul_div: { // multiplication and division templates
        md_1: {
            global_reqs(a,b,c,d) {
                return (
                    d > 0 &&
...
export default async function genRatEx(settings) {
    ...
    if (operation_type === 'add') {
        current_Q_obj = REH.add_sub[ratex_add_sub_form];
        ...
        current_Q_obj = REH.mul_div[ratex_mul_div_form];
    ...
    while (++current_regens <= max_regens) {
        if (!sol_is_found) { // use a backup template if a sol wasn't found in time OR if we jumped back here after finding NaN, Infinity, etc
            if (backup_json === null) backup_json = (await import('../backup-jsons/bkpRatEx.js')).default;
            final_coef_arr = H.randFromList(backup_json[current_form_name][operation_type]);
        }
